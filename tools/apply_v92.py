...
# game.js: add independent hit-stun state and block player controls while flinching.
replace_once(
    'js/game.js',
...
replace_once(
    'js/game.js',
    'lastShot:-1e9,lock:0,directionLocked:false',
    'lastShot:-1e9,lock:0,hitStunUntil:0,directionLocked:false'
)
replace_once(
    'js/game.js',
    "function useChip(){if(s.paused||s.dash>0||s.lock>0||!s.queue.length||editMode)return;",
    "function useChip(){if(s.paused||s.dash>0||s.lock>0||isHitStunned()||!s.queue.length||editMode)return;"
)
replace_once(
    'js/game.js',
    "A.onpointerdown=e=>{if(editMode)return;e.preventDefault();A.classList.add('pressed');useChip()};",
...
replace_once(
    'js/game.js',
    "}else if(s.lock<=0){s.x+=s.ix*SPEED*dt;s.y+=s.iy*SPEED*dt}}",
    "}else if(s.lock<=0&&!isHitStunned(t)){s.x+=s.ix*SPEED*dt;s.y+=s.iy*SPEED*dt}}"
)
replace_once(
    'js/game.js',
...
# Successful non-lethal damage starts both flinch and the existing 2s short-invincibility.
replace_once(
...
# Formal gameplay design: flinch and spatial knockback are separate.
replace_once(
    'GAME_DESIGN.md',
...
# Development history and next phase.
status_path = Path('DEVELOPMENT_STATUS.md')
status = status_path.read_text(encoding='utf-8')
v91_marker = "v91で原作EXE4の被弾後ショートインビジを参考に、通常被弾が成立した直後から `2秒` の被弾後無敵を `player-damage-system.js` へ追加した。"
pos = status.find(v91_marker)
if pos < 0:
    raise SystemExit('DEVELOPMENT_STATUS.md: v91 marker not found')
