...
AI_COLLECTION_NAME = "AI_CANDIDATE"
AI_ROOT_NAME = "AI_NEXIA_ROOT"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Import and align the Hunyuan Nexia candidate for reference comparison."
    )
    parser.add_argument(
        "--blend",
    ...
    parser.add_argument(
        ...
        help="Hunyuan-generated GLB to import.",
    )
    parser.add_argument(
        "--yaw",
        type=float,
...
def clear_previous_candidate():
    collection = bpy.data.collections.get(AI_COLLECTION_NAME)
    ...
    for mesh in list(bpy.data.meshes):
        if mesh.get("nexia_ai_candidate") and mesh.users == 0:
            bpy.data.meshes.remove(mesh)

    for material in list(bpy.data.materials):
        if material.get("nexia_ai_candidate") and material.users == 0:
            bpy.data.materials.remove(material)


def create_candidate_collection():
    collection = bpy.data.collections.new(AI_COLLECTION_NAME)
...
def mark_candidate_data(obj):
    obj["nexia_ai_candidate"] = True

    if obj.data is not None:
        try:
            obj.data["nexia_ai_candidate"] = True
        except TypeError:
            pass
    ...
    if obj.type == "MESH" and obj.data is not None:
        for material in obj.data.materials:
            if material is not None:
                material["nexia_ai_candidate"] = True
...
def fit_candidate(root, mesh_objects, reference_fit, yaw_degrees):
    ...
    if tallest_axis != "Z":
        raise RuntimeError(
            "Imported GLB is not Z-up after Blender glTF import. "
    ...
    if before_extent.z <= 0.0:
        raise RuntimeError(f"Imported candidate height is invalid: {before_extent.z}")
...
def main():
    ...
    if not os.path.isfile(glb_path):
        raise FileNotFoundError(
            "AI candidate GLB not found: "
            f"{glb_path}\n"
            "Place the generated file there or pass --glb <path>."
        )
    ...
    root = bpy.data.objects.new(AI_ROOT_NAME, None)
    root.empty_display_type = "PLAIN_AXES"
    root.empty_display_size = 0.12
    root["nexia_ai_candidate"] = True
    ...
    scene["nexia_ai_candidate_source"] = bpy.path.relpath(glb_path)
    scene["nexia_ai_candidate_collection"] = AI_COLLECTION_NAME
    scene["nexia_ai_candidate_yaw_deg"] = float(args.yaw)
    scene["nexia_ai_candidate_fit_scale"] = fit_info["uniform_scale"]
    scene["nexia_ai_candidate_status"] = "REFERENCE_COMPARISON_PENDING"
    ...
    print(f"Collection: {AI_COLLECTION_NAME}")
