...
同時にCIRCLEプレビューを多頂点ポリゴンではなく投影後のSVG楕円で描画し、カスタムゲージ更新時の二重Range描画も解消した。v46の実機での負荷・放物線表示は確認待ち。
...
v46のミニボム表示・負荷確認後、共通Rangeを実際の敵Hit判定へ接続できる基礎を作る。

優先対象は以下。

1. v46のミニボム `0.75マス / 3マス投擲 / 放物線` と操作負荷を実機確認する。
