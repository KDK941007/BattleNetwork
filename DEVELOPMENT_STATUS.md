...
v50では30fps制限を撤回し、LINE / RECTはローカル座標のpolygonを固定したままSVG `matrix()` transformで位置・向きを更新する方式へ変更した。CIRCLEは半径形状を固定し、中心位置のみtranslateで更新する。ミニボム放物線もローカルpathとtranslateを分離し、移動時のpath再生成を抑える。CUSTOMゲージはFillをblock表示へ戻して幅アニメーションを復旧し、見た目を変えずに疑似要素でタップ領域だけを上下左右へ拡張した。v50は実機確認待ち。
...
- v50のRange表示中の操作負荷を実機再確認。
- v50のCUSTOMゲージ初回タップ動作を実機再確認。
- v50でCUSTOMゲージ蓄積アニメーションが復旧しているか実機再確認。
- 特殊地形の実ゲーム処理。
...
v46のミニボム見た目は実機確認済み。v50の操作負荷、CUSTOMゲージ初回タップ、蓄積アニメーションを確認し、問題が解消した後、共通Rangeを実際の敵Hit判定へ接続できる基礎を作る。

優先対象は以下。

1. v50でキャノン・ミニボム等のRange表示中に移動・旋回が重くならないか実機再確認する。
2. v50でCUSTOMゲージMAX後の初回タップでCUSTOM画面が開くか実機再確認する。
3. v50でCUSTOMゲージの蓄積アニメーションが復旧しているか実機再確認する。
4. 敵キャラクターの最小HitBoxを追加する。
5. `BattleNetworkRangeGeometry` と敵HitBoxの連続座標判定を接続する。
6. プレイヤーHP・敵HP・ダメージ処理の最小基盤へ進む。
7. その後、敵AI・被弾・撃破・Wave進行へ拡張する。