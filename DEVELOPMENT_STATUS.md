...
v62でv61値からさらに2倍へ拡大し、プレイヤー表示を108×136px、テスト敵表示を116×140pxへ変更した。プレイヤー・敵HitBoxも `0.66マス` 四方から `1.32マス` 四方（237.6 world units）へ倍化した。表示とHitBoxの分離、足元world座標、既存Range Geometry・弾速・攻撃Rangeは変更していない。v62は実機確認待ち。
...
- v62のプレイヤー既定表示は108×136px、既定HitBoxは `1.32マス × 1.32マス`。
...
- v62のテスト敵既定表示は116×140px、既定HitBoxは `1.32マス × 1.32マス`。
...
- v62プレイヤー／敵の表示サイズとHitBox倍化の実機確認。
- 特殊地形の実ゲーム処理。
...
敵HitBox、バトルチップBehavior命中タイミング、通常/チャージロックバスターの敵HitBox接続、弾の高度補正と影位置まで実機確認済み。
まずv62のプレイヤー／敵サイズとHitBox倍化を実機確認し、問題なければ敵HP・ダメージ処理の最小基盤へ進む。

優先対象は以下。

1. v62プレイヤー108×136px、テスト敵116×140px、両HitBox `1.32×1.32マス` の見た目と命中感を実機確認する。
2. 敵HPの最小基盤を追加する。
