...
床表示はリポジトリ反映済みで、実機でのマスサイズ・視認性確認待ち。
...
- 見た目の1マスと内部の1マスを1対1で一致させる構成まで実装済み。実機視認性は確認待ち。
...
論理グリッド基盤と床表示まで実装したため、実機でマスサイズ・視認性を確認してからゲームロジックへ接続する。

優先対象は以下。

1. 20×20床表示の実機確認。マスサイズ・視認性を確認する。
2. プレイヤー所属マス取得をゲームループへ接続する。
3. Rangeのマス単位→world units変換処理をチップ処理へ接続する。
