...
v63でカメラをもう少し引いた見え方を比較するため、`js/ui/camera-zoom-comparison.js` を一時追加した。候補は `0.58 / 0.54 / 0.50` の3段階で、初期値は0.58。Yボタンを押すごとに `0.58 → 0.54 → 0.50 → 0.58` と循環し、画面右上に候補番号と倍率を表示する。これは比較専用実装であり、正式カメラ倍率とYボタン用途は未確定。
...
- X: 緊急回避ダッシュを実装済み。
- Y: v63ではカメラ倍率3候補の一時比較切替に使用。正式用途は未確定。
...
- v63では正式値決定用としてカメラ倍率 `0.58 / 0.54 / 0.50` をYボタンで一時比較できる。
...
- v63カメラ倍率 `0.58 / 0.54 / 0.50` の実機比較と正式値の確定。
- 特殊地形の実ゲーム処理。
...
## 次フェーズ: カメラ倍率確定 → 敵HP・ダメージ基盤

v62のプレイヤー／敵表示サイズは実機確認済み。
まずv63の3候補から正式カメラ倍率を決め、一時比較用Y操作と比較表示を撤去して正式値へ固定する。その後、敵HP・ダメージ処理の最小基盤へ進む。

優先対象は以下。

1. Yボタンで `0.58 / 0.54 / 0.50` を実機比較し、正式カメラ倍率を決定する。
2. 決定後に一時比較用スクリプト・表示・Y操作を撤去し、正式倍率へ固定する。
3. 敵HPの最小基盤を追加する。
