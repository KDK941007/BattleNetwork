...
v48で毎フレーム実行していた `battle.getBoundingClientRect()` を廃止し、battleサイズは初期化時とresize時だけ取得するよう変更した。スティックもpointermoveごとの `joy.getBoundingClientRect()` を廃止し、pointerdown時に取得した矩形をドラッグ中に再利用する。所属マスの `data-*` 属性更新もrow/col/terrainが変化した時だけ実施する。CUSTOMゲージは共通ハンドラを `pointerdown` と `click` の双方へ接続し、二重発火を防ぎつつ初回タップの取りこぼしを補完する。v48は実機確認待ち。
...
- v48のRange表示中の操作負荷を実機再確認。
- v48のCUSTOMゲージ初回タップ動作を実機再確認。
- 特殊地形の実ゲーム処理。
...
v46のミニボム見た目は実機確認済み。v48の操作負荷とCUSTOMゲージ初回タップを確認し、問題が解消した後、共通Rangeを実際の敵Hit判定へ接続できる基礎を作る。

優先対象は以下。

1. v48でキャノン・ミニボム等のRange表示中に移動・旋回が重くならないか実機再確認する。
2. v48でCUSTOMゲージMAX後の初回タップでCUSTOM画面が開くか実機再確認する。
