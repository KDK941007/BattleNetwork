...
v66でカメラ倍率下限を `0.20` まで拡張し、現行設定範囲を `0.20〜0.62`、0.01刻みとした。またワイドソード画像の正式ファイル名変更 `WideSwordpng.png` → `WideSword.png` に合わせ、`chip-master.js` の画像参照とService Workerのキャッシュ対象を `assets/chips/WideSword.png` へ更新した。v66は実機確認待ち。
...
- v66でコントローラー配置編集画面からカメラ倍率を `0.20〜0.62`、0.01刻みで変更・保存できる。
...
- v66カメラ倍率 `0.20〜0.62` の操作性・保存・見え方の実機確認。
- v66ワイドソード画像の再表示確認。
- 特殊地形の実ゲーム処理。
...
## 次フェーズ: 操作・カメラ確認 → 敵HP・ダメージ基盤

v62のプレイヤー／敵表示サイズは実機確認済み。
v66でカメラ倍率下限を0.20へ拡張し、ワイドソードの画像参照を正式ファイル名 `WideSword.png` へ修正した。
まずv65/v66の操作・表示を実機確認し、問題なければ敵HP・ダメージ処理の最小基盤へ進む。

優先対象は以下。

1. 配置編集バーがCUSTOMゲージと重なる位置で崩れず、横幅も十分に確保されることを確認する。
2. カメラ倍率スライダーが `0.20〜0.62` で即時反映・保存されることを確認する。
3. ワイドソードのイラストがCUSTOM画面・詳細画面で再表示されることを確認する。
4. Yを1回タップするとその時点の向きが固定され、移動しても向きが変わらず、A/B攻撃が固定方向へ出ることを確認する。
5. Y固定中の色変化と、再タップでの固定解除を確認する。
6. 敵HPの最小基盤を追加する。
7. バトルチップ / ロックバスターのダメージ処理を接続する。
8. プレイヤーHPへ進む。
9. その後、敵AI・被弾・撃破・Wave進行へ拡張する。
