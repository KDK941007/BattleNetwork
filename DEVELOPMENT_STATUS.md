...
v64で `js/ui/camera-zoom-settings.js` を追加し、コントローラー配置編集画面の上部バーへカメラ倍率スライダーを常設した。倍率は `0.50〜0.62` を `0.01` 刻みで変更でき、初期値は `0.58`。変更は即時にフィールドへ反映し、`localStorage` の `battleNetworkCameraZoomV1` へ保存する。「デフォルトに戻す」で倍率も0.58へ戻る。v63の一時比較スクリプト・右上比較表示・Yボタン切替は削除し、Yボタンは再び未使用へ戻した。v64は実機確認待ち。
...
- v64でコントローラー配置編集画面からカメラ倍率を `0.50〜0.62`、0.01刻みで変更・保存できる。
...
- カメラ倍率は固定値にせず、端末設定として `0.50〜0.62` の範囲を0.01刻みでユーザー調整可能とする。
...
## 次フェーズ: カメラ設定確認 → 敵HP・ダメージ基盤

v62のプレイヤー／敵表示サイズは実機確認済み。
v64で固定倍率選択を廃止し、コントローラー配置編集画面からカメラ倍率を連続調整・保存できる方式へ変更した。
まずスライダー操作・保存・実際の見え方を実機確認し、問題なければ敵HP・ダメージ処理の最小基盤へ進む。

優先対象は以下。

1. カメラ倍率スライダーを動かし、0.50〜0.62の範囲で即時反映されることを実機確認する。
2. 設定画面を閉じた後・再読み込み後も選択倍率が維持されることを確認する。
3. 敵HPの最小基盤を追加する。
4. バトルチップ / ロックバスターのダメージ処理を接続する。
5. プレイヤーHPへ進む。
