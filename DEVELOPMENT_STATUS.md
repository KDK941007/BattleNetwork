...
## 次フェーズ: v83 HUD実機確認 → プレイヤーHP具体値・被ダメージ接続

v82で左上HP表示とココロウィンドウの表示基盤を追加したが、実機ではHUDが大きくCUSTOMゲージ左端と干渉することを確認した。v83でHUD全体を小型化したため、まず実機でCUSTOMゲージと重ならないこと、HP数字とココロウィンドウの視認性を確認する。問題なければプレイヤー初期最大HPの具体値を確定したうえで敵攻撃からの被ダメージへ接続する。リカバリー10の実回復はプレイヤーHP仕様確定後に接続する。
