...
同日、`js/field/field-grid.js` を追加し、20×20論理グリッドの内部基盤を実装した。現時点では見た目の床グリッド、プレイヤー所属マス、既存チップRangeにはまだ接続していない。
...
- 現在の床グリッド風表示はまだ装飾であり、論理グリッド描画には未接続。
...
- 論理グリッドと一致する床描画。
- プレイヤー所属マス取得とゲーム側への接続。
- 特殊地形の実ゲーム処理。
- Rangeのマス単位→world units共通変換を既存チップ処理へ接続。
- 既存Rangeマスタの `LINE / RECT / CIRCLE / SELF` への実データ移行。
...
20×20論理グリッドの内部基盤は実装済み。次は見た目とゲームロジックを段階的に接続する。

優先対象は以下。

1. 床表示を論理グリッドと一致させる。
2. プレイヤー所属マス取得をゲーム側へ接続する。
3. Rangeのマス単位→world units変換処理を既存チップ処理へ接続する。
4. 既存5チップのRangeマスタを新Type・新Parameterへ移行する。
5. 既存5チップの攻撃範囲・プレビューを新方式へ移行する。
6. その後、HP・ダメージ・敵HitBox・正式な攻撃判定へ進む。
