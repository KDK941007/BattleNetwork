...
## 次フェーズ: v84 HUD実機確認 → プレイヤーHP具体値・被ダメージ接続

v82で左上HP表示とココロウィンドウの表示基盤を追加した。v83で小型化した後も実機ではCUSTOMゲージ左端との干渉が残ったため、v84でHUD最大幅を88pxへ縮小し左位置も詰めた。まず実機でCUSTOMゲージと完全に重ならないこと、HP数字とココロウィンドウの視認性を確認する。問題なければプレイヤー初期最大HPの具体値を確定したうえで敵攻撃からの被ダメージへ接続する。リカバリー10の実回復はプレイヤーHP仕様確定後に接続する。
