...
v49ではRangeプレビューのSVG座標更新を最大約30fpsへ制限し、ゲーム本体の60fps相当ループやA押下時の正式Range Geometryとは分離した。キャノンのpolygonとミニボムのellipse／放物線pathのDOM更新頻度を抑える。CUSTOMゲージは `div role=button` からネイティブの `button` 要素へ変更し、ブラウザ標準のタップ処理を利用する。v49は実機確認待ち。
...
- v49でRangeプレビュー表示のDOM更新を最大約30fpsへ制限。正式Range GeometryとA押下時の攻撃判定用Range生成は制限しない。
- v49でミニボム放物線プレビューも同じ更新間隔へ制限。
...
- v49でCUSTOMゲージ本体をネイティブ `button` 要素へ変更。既存の見た目とIDは維持する。
...
- v49のRange表示中の操作負荷を実機再確認。
- v49のCUSTOMゲージ初回タップ動作を実機再確認。
- 特殊地形の実ゲーム処理。
...
v46のミニボム見た目は実機確認済み。v49の操作負荷とCUSTOMゲージ初回タップを確認し、問題が解消した後、共通Rangeを実際の敵Hit判定へ接続できる基礎を作る。

優先対象は以下。

1. v49でキャノン・ミニボム等のRange表示中に移動・旋回が重くならないか実機再確認する。
2. v49でCUSTOMゲージMAX後の初回タップでCUSTOM画面が開くか実機再確認する。
3. 敵キャラクターの最小HitBoxを追加する。
4. `BattleNetworkRangeGeometry` と敵HitBoxの連続座標判定を接続する。
5. プレイヤーHP・敵HP・ダメージ処理の最小基盤へ進む。
