...
v59でキャノン表示を132×66pxへ拡大し、影を114×39pxへ変更。正式Rangeを `LINE(5×0.75)`、幅135 world unitsへ拡大した。射程5マス、弾速、攻撃力は変更していない。またRange表示が実攻撃を隠しやすかったため、Range SVG全体をopacity 0.45、ミニボム投擲軌道をopacity 0.55へ透明化した。v59は実機確認待ち。
...
- v59キャノン表示サイズ・影・Range幅・Range透明度の実機確認。
- 特殊地形の実ゲーム処理。
...
まずv59のキャノン表示サイズ・Range幅・Range透明度を実機確認し、問題なければ敵HP・ダメージ処理の最小基盤へ進む。

優先対象は以下。

1. v59キャノンの表示サイズ132×66px、影114×39px、Range `5×0.75` とRange透明度の見た目・命中感を実機確認する。
2. 敵HPの最小基盤を追加する。
