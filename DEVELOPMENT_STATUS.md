...
## 次フェーズ: 敵HP表示位置比較 → プレイヤーHP

v71までのチップ詳細Range図は実機確認済み。v72で敵HP基盤、v73でテスト敵HP200・チップ/B攻撃の実ダメージ接続と差し替え可能なB攻撃基盤を追加し、実機確認済み。v74では残HP表示を原作寄りへ変更し、上表示・下表示を同時に出して比較する。正式位置を決定後、プレイヤーHPへ進む。
