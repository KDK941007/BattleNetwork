...
v60で弾サイズに依存しない表示補正へ変更し、キャノン / 通常バスター / チャージショットの見た目中心を正式world座標へ合わせた。弾の下端は床面から約20px浮くよう実サイズから自動補正し、床影も弾サイズではなく正式な床面world座標を基準に配置する。命中判定・弾速・Range値は変更していない。v60は実機確認待ち。
...
- v60で弾の表示中心をworld座標へ合わせ、実サイズから床面とのクリアランスを自動補正する方式へ変更した。
...
- v60キャノン / 通常バスター / チャージショットの高度補正と影位置の実機確認。
- 特殊地形の実ゲーム処理。
...
まずv60の弾高度補正と影位置を実機確認し、問題なければ敵HP・ダメージ処理の最小基盤へ進む。

優先対象は以下。

1. v60キャノン / 通常バスター / チャージショットの弾中心・床面クリアランス・影位置を実機確認する。
2. 敵HPの最小基盤を追加する。
