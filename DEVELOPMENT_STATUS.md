...
v61でプレイヤー表示を44×56pxから54×68px、テスト敵表示を48×58pxから58×70pxへ一回り拡大した。敵HitBoxは `0.55マス` 四方から `0.66マス` 四方（118.8 world units）へ拡大。プレイヤーにも表示と分離した `0.66マス` 四方のworld矩形HitBox基盤を追加し、`BattleNetworkPlayer` から位置・HitBox境界・点包含判定を参照できるようにした。v61は実機確認待ち。
...
- v61でプレイヤー表示を54×68pxへ拡大し、足元world座標を維持したまま描画位置を再計算する構成へ変更。
- v61でプレイヤーHitBoxを `0.66マス × 0.66マス` として追加し、表示サイズとは独立管理する。
...
- v61のテスト敵既定表示は58×70px、既定HitBoxは `0.66マス × 0.66マス`。
...
- v61プロトタイプではプレイヤー・テスト敵のHitBox既定値をともに `0.66マス × 0.66マス` とする。
...
- v61プレイヤー／敵の表示サイズとHitBox拡大の実機確認。
- 特殊地形の実ゲーム処理。
...
まずv61のプレイヤー／敵サイズとHitBox拡大を実機確認し、問題なければ敵HP・ダメージ処理の最小基盤へ進む。

優先対象は以下。

1. v61プレイヤー54×68px、テスト敵58×70px、両HitBox `0.66×0.66マス` の見た目と命中感を実機確認する。
2. 敵HPの最小基盤を追加する。
