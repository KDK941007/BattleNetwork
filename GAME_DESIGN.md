...
- 知覚開始距離の共通デフォルト値は `4マス` とする。敵固有の知覚開始距離を設定していない場合は4マスを使用し、敵タイプごとに個別値で上書き可能とする。
...
- 知覚解除距離の具体値はまだ確定しない。知覚開始距離・知覚解除距離ともに敵タイプごとの個別設定を可能とする。
...
- 知覚開始距離は敵共通デフォルトの `4マス` を使用する。HP、攻撃力、移動速度、待機時間、予兆時間、攻撃間隔、知覚解除距離等の具体値はまだ確定しない。
