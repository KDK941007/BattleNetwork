...
- 知覚開始距離の共通デフォルト値は `5マス` とする。敵固有の知覚開始距離を設定していない場合は5マスを使用し、敵タイプごとに個別値で上書き可能とする。
- 知覚解除距離の共通デフォルト値は `6マス` とする。敵固有の知覚解除距離を設定していない場合は6マスを使用し、敵タイプごとに個別値で上書き可能とする。
- プレイヤーを知覚していない間の共通デフォルト行動は、プレイヤーを直接追跡・攻撃せず、フィールド内を自由移動するものとする。未知覚時の移動方法は敵タイプごとに個別上書き可能とし、固定砲台のような移動しない敵、巡回ルートを持つ敵、待機中心の敵などは敵固有仕様として変更できる。
...
- 知覚開始距離・知覚解除距離ともに敵タイプごとの個別設定を可能とする。
...
- 移動はあり。プレイヤーを知覚していない間は、プレイヤーを直接追跡せずフィールド内を自由移動する。未知覚時の具体的な移動方向・移動範囲・速度・停止時間は後続設計で確定する。
- プレイヤーを知覚した後は、プレイヤーへ近づく追跡移動を行う。追跡時の移動速度・距離調整・停止条件、および追跡限界距離を使用するかどうかは後続設計で確定する。
...
- 知覚開始距離は敵共通デフォルトの `5マス`、知覚解除距離は敵共通デフォルトの `6マス` を使用する。HP、攻撃力、移動速度、待機時間、予兆時間、攻撃間隔等の具体値はまだ確定しない。
