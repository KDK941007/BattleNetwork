(()=>{
  ...
  function installEffect(){
    ...
    function launchBomb(context){
      ...
      const worldDx=targetWorld.x-originWorld.x,worldDy=targetWorld.y-originWorld.y,horizontal=Math.abs(worldDx)>=Math.abs(worldDy),spinSign=horizontal?(worldDx>=0?1:-1):(worldDy>=0?1:-1);
      ...
      const arcFrames=times.map(t=>{const x=sx+dx*t,y=sy+dy*t-4*arcHeight*t*(1-t),scale=.88+.10*(1-Math.abs(t*2-1)),rotation=spinSign*1080*t;if(horizontal)return{transform:`translate3d(${x}px,${y}px,0) rotate(${rotation}deg) scale(${scale})`,opacity:1,offset:t};const phase=rotation*Math.PI/180,depth=Math.sin(phase),roundness=Math.abs(Math.cos(phase)),scaleX=scale*(.82+.18*roundness),scaleY=scale*(.72+.28*roundness),rollY=depth*7,tilt=depth*5;return{transform:`translate3d(${x}px,${y+rollY}px,0) rotate(${tilt}deg) scale(${scaleX},${scaleY})`,filter:`brightness(${1+depth*.14}) drop-shadow(0 ${4-depth*2}px ${3+roundness*2}px rgba(0,0,0,.55)) drop-shadow(0 0 8px rgba(76,183,255,.72))`,opacity:1,offset:t}});
