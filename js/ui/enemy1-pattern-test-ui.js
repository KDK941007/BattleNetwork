(()=>{
  ...
  wrap.style.cssText='position:absolute;right:10px;top:10px;z-index:40;display:flex;align-items:center;gap:6px;padding:5px 7px;border:1px solid rgba(255,255,255,.5);border-radius:8px;background:rgba(8,12,20,.82);color:#fff;font:700 11px/1.2 system-ui,sans-serif;pointer-events:auto;';
  ...
  toggle.style.minWidth='82px';patternButton.style.minWidth='92px';tools.style.cssText='display:flex;align-items:center;gap:6px;';detail.style.cssText='white-space:nowrap;font-variant-numeric:tabular-nums;';

  const layer=document.createElement('div');
  layer.dataset.testOnly='enemy-perception-layer';
  layer.style.cssText=`position:absolute;left:0;top:0;width:${SW}px;height:${SH}px;transform-origin:0 0;pointer-events:none;z-index:6;display:none;will-change:transform;contain:layout style paint;`;
  const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');
  svg.setAttribute('viewBox',`0 0 ${SW} ${SH}`);svg.style.cssText='position:absolute;inset:0;width:100%;height:100%;overflow:visible;pointer-events:none;';
  ...
  function ensureRings(enemyId,startRadius,releaseRadius){let pair=rings.get(enemyId);if(pair)return pair;const start=createEllipse(startRadius,'rgba(255,211,82,.95)','rgba(255,211,82,.07)','16 10',5),release=createEllipse(releaseRadius,'rgba(84,235,255,.9)','none','9 12',4);svg.append(start,release);pair={start,release,x:NaN,y:NaN,aware:null};rings.set(enemyId,pair);return pair}
  ...
  function syncLayerTransform(){const next=scene.style.transform||'';if(next!==lastSceneTransform){layer.style.transform=next;lastSceneTransform=next}}
  function renderRanges(){const debug=RUNTIME.getDebugState();if(!debug.enabled||!debug.showPerception)return;syncLayerTransform();const config=RUNTIME.getEnemyConfig(),startRadius=FIELD.toWorldDistance(config.perceptionStartTiles),releaseRadius=FIELD.toWorldDistance(config.perceptionReleaseTiles),activeIds=new Set();for(const enemy of ENEMY.getActiveEnemies()){activeIds.add(enemy.id);const pair=ensureRings(enemy.id,startRadius,releaseRadius),aware=RUNTIME.getPerception(enemy.id),moved=Math.abs(pair.x-enemy.x)>=POSITION_EPSILON||Math.abs(pair.y-enemy.y)>=POSITION_EPSILON;if(moved){const p=project(enemy.x,enemy.y),transform=`translate(${p.x} ${p.y})`;pair.start.setAttribute('transform',transform);pair.release.setAttribute('transform',transform);pair.x=enemy.x;pair.y=enemy.y}if(pair.aware!==aware){pair.start.setAttribute('opacity',aware?'.55':'1');pair.release.setAttribute('opacity',aware?'1':'.5');pair.aware=aware}}clearStale(activeIds)}
  function stopRangeUpdates(){if(rangeTimer!==null){clearInterval(rangeTimer);rangeTimer=null}layer.style.display='none'}
  function startRangeUpdates(){if(rangeTimer!==null)return;layer.style.display='block';renderRanges();rangeTimer=setInterval(renderRanges,DEBUG_INTERVAL_MS)}
  ...
  RUNTIME.subscribeDebug(render);tools.append(patternButton,rangeButton,glowButton,detail);wrap.append(toggle,tools);battle.appendChild(wrap);
})();
