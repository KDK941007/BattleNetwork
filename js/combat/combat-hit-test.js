(()=>{
  ...
  function airShotSpeed(){return 4000}
  function vulcan1Speed(){return 4000}
  function spreadGunSpeed(){return 4000}
  ...
  function scheduleCannon(attack){const airShot=isAirShot(attack),vulcan1=isVulcan1(attack),spreadGun=isSpreadGun(attack),cannon=isCannon(attack);const speed=airShot?airShotSpeed():vulcan1?vulcan1Speed():spreadGun?spreadGunSpeed():cannon?cannonSpeed():behaviorParam('CANNON_SHOT','PROJECTILE_SPEED',2000);if(!(speed>0))return;const first=getFirstCannonHit(attack);if(!first)return;setTimeout(()=>{const result=damageAndFlash(first.enemy,attack.damage);if(airShot&&!result?.defeatedNow)pushAirShotEnemy(first.enemy.id,attack.shape.direction);if(spreadGun)window.BattleNetworkSpreadGun?.onDirectHit?.(attack,first.enemy)},first.distance/speed*1000)}
