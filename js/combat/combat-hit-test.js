(()=>{
  ...
  function airShotSpeed(){const value=Number(window.BattleNetworkTestSettings?.getAirShotSettings?.().projectileSpeed);return Number.isFinite(value)&&value>0?value:4000}
  ...
  function scheduleCannon(attack){const airShot=isAirShot(attack);const cannon=isCannon(attack);const speed=airShot?airShotSpeed():cannon?cannonSpeed():behaviorParam('CANNON_SHOT','PROJECTILE_SPEED',2000);if(!(speed>0))return;const first=getFirstCannonHit(attack);if(!first)return;setTimeout(()=>{const result=damageAndFlash(first.enemy,attack.damage);if(airShot&&!result?.defeatedNow)pushAirShotEnemy(first.enemy.id,attack.shape.direction)},first.distance/speed*1000)}
