(()=>{
  ...
  function scheduleCannon(attack){const airShot=isAirShot(attack);const speed=airShot?airShotSpeed():behaviorParam('CANNON_SHOT','PROJECTILE_SPEED',900);if(!(speed>0))return;const first=getFirstCannonHit(attack);if(!first)return;setTimeout(()=>{const result=damageAndFlash(first.enemy,attack.damage);if(airShot&&!result?.defeatedNow)pushAirShotEnemy(first.enemy.id,attack.shape.direction)},first.distance/speed*1000)}
