(()=>{
  ...
  function showExplosionCells(shapes){
    ...
    return perf('spreadVisual',()=>{
      ...
      if(hideTimer!==null)clearTimeout(hideTimer);hideTimer=setTimeout(()=>{svg.style.display='none';hideTimer=null},VISUAL_MS);
    });
  }
  function onDirectHit(attack,directEnemy){
    ...
    const result=perf('directHit',()=>{
      ...
      const hits=perf('spreadCalc',()=>RELATIVE.getHitEnemies(shapes,{excludeIds:[directEnemy.id]}));const damage=Number(attack.damage);
    ...
    mark('spread nextFrame');return result;
  }
