(()=>{
  ...
  function createController({enemyId}){
    ...
    function moveToResult(enemy,x,y){
      if(enemy.collision?.allowPlayerOverlap!==true&&ENEMY.wouldOverlapBounds?.(enemyId,x,y,PLAYER.getBounds()))return{applied:false,reason:'PLAYER_COLLISION',enemy};
      const result=ENEMY.setPosition(enemyId,x,y);
      if(result?.applied){lastX=result.enemy.x;lastY=result.enemy.y}
      return result||{applied:false,reason:'MOVE_FAILED',enemy};
    }
    function moveTo(enemy,x,y){return moveToResult(enemy,x,y)?.applied===true}
    function isCrowdBlock(result){return result?.reason==='ENEMY_COLLISION'||result?.reason==='PLAYER_COLLISION'}
    function buildPath(player,now){
      const finder=NAV?.findPathBudgeted||NAV?.findPath;
      if(!finder)return false;
      const next=finder(enemyId,player,{blockedBounds:PLAYER.getBounds(),ignoreEnemies:true});
      if(next===null){lastPathAt=now-PATH_RECALC_MS+PATH_RETRY_MS;return false}
      path=[...next];pathTargetKey=tileKeyAt(player);lastPathAt=now;return path.length>0
    }
    function followPath(enemy,player,speed,dt,now){
      if(now<crowdBlockedUntil)return false;
      ...
      const waypoint=path[0],d=unit(waypoint.x-enemy.x,waypoint.y-enemy.y),result=moveToResult(enemy,enemy.x+d.x*speed*dt,enemy.y+d.y*speed*dt);
      if(result?.applied)return true;
      if(isCrowdBlock(result)){crowdBlockedUntil=now+CROWD_RETRY_MS;return false}
      if(result?.reason==='TERRAIN_BLOCKED'&&now-lastPathAt>=PATH_RECALC_MS){clearPath();buildPath(player,now)}
      return false;
    }
    function chase(enemy,player,speed,dt,now){
      ...
      if(now<crowdBlockedUntil)return;
      if(config.navigationPolicy===RUNTIME.NAVIGATION_POLICY?.DIRECT||!NAV){clearPath();const result=moveToResult(enemy,enemy.x+d.x*speed*dt,enemy.y+d.y*speed*dt);if(isCrowdBlock(result))crowdBlockedUntil=now+CROWD_RETRY_MS;return}
      if(path.length){followPath(enemy,player,speed,dt,now);return}
      const result=moveToResult(enemy,enemy.x+d.x*speed*dt,enemy.y+d.y*speed*dt);
      if(result?.applied){pathTargetKey=tileKeyAt(player);return}
      if(isCrowdBlock(result)){crowdBlockedUntil=now+CROWD_RETRY_MS;return}
      if(result?.reason==='TERRAIN_BLOCKED'&&now-lastPathAt>=PATH_RECALC_MS&&buildPath(player,now))followPath(enemy,player,speed,dt,now);
    }
    ...
    function cancel(){active=false;target=null;clearPath();crowdBlockedUntil=0}
  ...
  window.BattleNetworkEnemy1Movement=Object.freeze({BEHAVIOR_ID,PUSH_STUN_MS,PATH_RECALC_MS,PATH_RETRY_MS,CROWD_RETRY_MS});
})();