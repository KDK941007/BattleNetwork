(()=>{
  ...
  function createController({enemyId}){
    let active=false,target=null,waitUntil=0,lastX=null,lastY=null,pushStunUntil=0,path=[],pathTargetKey='',lastPathAt=performance.now()-PATH_RECALC_MS+pathPhaseOffset(enemyId);
    ...
    function buildPath(player,now){
      ...
      const next=finder(enemyId,player,{blockedBounds:PLAYER.getBounds()});
    ...
    function chase(enemy,player,speed,dt,now){
      ...
      if(now-lastPathAt>=PATH_RECALC_MS&&buildPath(player,now))followPath(enemy,player,speed,dt,now);
    }
  ...
  window.BattleNetworkEnemy1Movement=Object.freeze({BEHAVIOR_ID,PUSH_STUN_MS,PATH_RECALC_MS,PATH_RETRY_MS});
})();