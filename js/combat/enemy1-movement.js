(()=>{
  ...
  function createController({enemyId}){
    ...
    function moveTo(enemy,x,y){if(enemy.collision?.allowPlayerOverlap!==true&&ENEMY.wouldOverlapBounds?.(enemyId,x,y,PLAYER.getBounds()))return false;ENEMY.setPosition(enemyId,x,y);return true}
    function canStart(){const e=ENEMY.getEnemy(enemyId);return !!e&&!e.isDefeated}
    function start(){active=true;return true}
    function update(now,dt){const enemy=ENEMY.getEnemy(enemyId);if(!enemy||enemy.isDefeated){active=false;return}const player=PLAYER.getPosition(),aware=updatePerception(enemy,player);if(RUNTIME.isAttackLocked(enemyId))return;const speed=RUNTIME.getPattern().moveSpeedWorld;if(aware){target=null;if(!shouldChase(enemy,player,now))return;const d=chaseDirection(enemy,player);if(!d)return;moveTo(enemy,enemy.x+d.x*speed*dt,enemy.y+d.y*speed*dt);return}if(now<waitUntil)return;if(!target)chooseWander(enemy);const dx=target.x-enemy.x,dy=target.y-enemy.y,remain=Math.hypot(dx,dy);if(remain<=Math.max(3,speed*dt)){if(moveTo(enemy,target.x,target.y)){target=null;waitUntil=now+random(800,1500)}else{target=null;waitUntil=now+random(300,700)}return}const d=unit(dx,dy);if(!moveTo(enemy,enemy.x+d.x*speed*dt,enemy.y+d.y*speed*dt)){target=null;waitUntil=now+random(300,700)}}
    function cancel(){active=false;target=null}
    function destroy(){cancel();RUNTIME.clearEnemy(enemyId)}
    function isBusy(){return active}
    return Object.freeze({canStart,start,update,cancel,destroy,isBusy,getSnapshot:()=>Object.freeze({enemyId,behaviorId:BEHAVIOR_ID,aware:RUNTIME.getPerception(enemyId),chasePolicy:RUNTIME.getEnemyConfig().chasePolicy,chaseRangeTiles:RUNTIME.getEnemyConfig().chaseRangeTiles,chaseDistanceMode:RUNTIME.getEnemyConfig().chaseDistanceMode,approachStopTiles:RUNTIME.getEnemyConfig().approachStopTiles,keepDistanceMinTiles:RUNTIME.getEnemyConfig().keepDistanceMinTiles,keepDistanceMaxTiles:RUNTIME.getEnemyConfig().keepDistanceMaxTiles,busy:active})});
  }
