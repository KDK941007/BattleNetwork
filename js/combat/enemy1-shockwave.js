(()=>{
  ...
  function createController({enemyId}){
    ...
    function start(now){if(!canStart(now))return false;const enemy=ENEMY.getEnemy(enemyId),player=PLAYER.getPosition(),c=cfg();direction=unit(player.x-enemy.x,player.y-enemy.y);phase='TELEGRAPH';fireAt=now+c.telegraphMs;fullSyncAt=Math.max(now,fireAt-c.fullSyncWindowMs);RUNTIME.setAttackLocked(enemyId,true);const end={x:enemy.x+direction.x*FIELD.toWorldDistance(c.maxRangeTiles),y:enemy.y+direction.y*FIELD.toWorldDistance(c.maxRangeTiles)};LAYER.showTelegraph(telegraphEl,{x:enemy.x,y:enemy.y},end);setGlow('TELEGRAPH');return true}
    function fire(now){const enemy=ENEMY.getEnemy(enemyId);if(!enemy){cancel(now);return}LAYER.hideTelegraph(telegraphEl);setGlow('NONE');projectile={x:enemy.x,y:enemy.y,travel:0,maxTravel:FIELD.toWorldDistance(cfg().maxRangeTiles)};phase='PROJECTILE';LAYER.showProjectile(projectileEl,projectile.x,projectile.y)}
    ...
    function getSnapshot(){return Object.freeze({enemyId,behaviorId:BEHAVIOR_ID,phase,fullSyncActive:phase==='TELEGRAPH'&&performance.now()>=fullSyncAt,fullSyncAt,fireAt,nextAttackAt:RUNTIME.getNextAttackAt(enemyId),pattern:cfg()})}
