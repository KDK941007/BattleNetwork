(()=>{
  ...
  function render(now){
    ...
    lastRender=now;
    const fixed=['A task','firstHit','directHit','spreadCalc','spreadDamage','spreadVisual','frameGap'];
    ...
    for(const name of order){const v=values.get(name);if(v)lines.push(`${name.padEnd(14)} ${v.last.toFixed(1).padStart(6)} / ${v.max.toFixed(1).padStart(6)}`)}
  ...
  function monitorFrame(now){const gap=now-lastFrame;lastFrame=now;if(gap>20)record('frameGap',gap);render(now);nativeRAF(monitorFrame)}
