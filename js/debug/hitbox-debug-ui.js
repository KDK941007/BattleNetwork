(()=>{
  ...
  function setCircle(el,x,y,diameter){const c=project(x,y),r=diameter/2;el.setAttribute('cx',c.x);el.setAttribute('cy',c.y);el.setAttribute('rx',Math.SQRT2*r*PX);el.setAttribute('ry',Math.SQRT2*r*PY)}
  ...
  function draw(){frame=null;const enabled=requested&&RUNTIME.getDebugState().enabled;if(!enabled){svg.style.display='none';return}svg.style.display='block';const diameter=FIELD.toWorldDistance(diameterTiles),pp=PLAYER.getPosition();setCircle(playerEllipse,pp.x,pp.y,diameter);const active=ENEMY.getActiveEnemies(),ids=new Set();for(const enemy of active){ids.add(enemy.id);setCircle(ensureEnemyEllipse(enemy.id),enemy.x,enemy.y,diameter)}for(const [id,el] of enemyEllipses){if(!ids.has(id)){el.remove();enemyEllipses.delete(id)}}frame=requestAnimationFrame(draw)}
  ...
  sizeBox.style.cssText='display:flex;align-items:center;justify-content:space-between;gap:12px;min-height:38px;padding:2px 0;';label.textContent='判定直径';label.style.cssText='color:#c8f5ff;font-weight:900;min-width:76px;';control.style.cssText='display:inline-flex;align-items:center;gap:8px;white-space:nowrap;';
  for(const b of [prev,next]){b.type='button';b.style.cssText='width:42px;height:34px;padding:0;border:1px solid #8eeaff;border-radius:5px;background:#0c4053;color:#eaffff;font-weight:900;line-height:1;'}prev.textContent='◀';next.textContent='▶';value.style.cssText='display:inline-block;min-width:82px;text-align:center;color:#fff;font-size:13px;';
  ...
  window.BattleNetworkHitboxDebug=Object.freeze({LIMIT,setEnabled,getEnabled,setDiameterTiles,adjustDiameter,getDiameterTiles});
})();
